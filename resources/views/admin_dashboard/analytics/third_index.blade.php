    ...
    <div class="AdminAna">
      ...
      <div class="analyticsTopBtns"> 
          <div class="analyticsBtnsHub">
            ...
            <a href="/analytics2" style="color: white ; background-color: black;">Redeemed Offers</a>
        ...
        <h2>Redeemed offers</h2>
        <div class="graph">
            <div style="width: 70%;">
                <canvas  id="redeemedOffers" width="70" height="30"></canvas>
            </div>
        </div>
                    ...
                    <div class="col-md-9 cardSize" >
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            ...
                                            <tr style="font-size: 90%">
                                                <td>{{ $admintable->offer->merchants->business_name  }}</td>
                                                <td>{{ $admintable->user->name }}</td>
                                                <td>{{ $admintable->offer->name }}</td>
                                                <td>{{ $admintable->offer->type->name }}</td>
                                                <td>{{ $admintable->saving }}</td>
                                                <td>{{ $admintable->code }}</td>
                                                <td>{{ $admintable->created_at->format('d/m/Y')}}</td>
                                    ...
                                    <span>
                                        {{ $admintables->onEachSide(1)->links() }}
                                    </span>
                                </div>
                            </div>
...
<script  type="text/javascript">  
    ...
    $useroffers = json_decode($useroffers, true);

    for($i = 0;$i < count($useroffers);$i++){
      echo "useroffers[" . ($useroffers[$i]['month'] - 1) . "] = " . $useroffers[$i]['num'] . ";";
    }
